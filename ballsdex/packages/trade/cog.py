...
class Trade(commands.GroupCog):
    ...
    async def add(
        ...
        if not countryball.is_tradeable:
            await interaction.response.send_message(
                "You cannot trade this {settings.collectible_name}!", ephemeral=True
            )
            return
        ...
        if await countryball.is_locked():
            await interaction.followup.send(
                "This icon is currently in an active trade or donation, "
    ...
    async def bulk_add(
        ...
        await view.start(
            content="Select the icons you want to add to your proposal, "
            "note that the display will wipe on pagination however "
            "the selected icons will remain.\n"
            "Icons were rounded down to closest 25 for "
