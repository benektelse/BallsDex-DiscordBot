...
class Balls(commands.GroupCog, group_name=settings.players_group_cog_name):
    ...
    async def duplicate(
        ...
        """
        Shows your most duplicated icons or specials.

        Parameters
        ----------
        type: DuplicateType
            Type of duplicate to check (icons or specials).
        limit: int | None
            The amount of icons to show, can only be used with `icons`.
        """
    ...
    async def collection(
        ...
        """
        Show the collection of a specific icon.

        Parameters
        ----------
        countryball: Ball
            The icon you want to see the collection of
        """
        await interaction.response.defer(thinking=True)
        player, _ = await Player.get_or_create(discord_id=interaction.user.id)
