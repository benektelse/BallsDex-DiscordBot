...
activation_embed = discord.Embed(
    colour=0x00D936,
    title=f"IconDex activation",
    description=f"To enable IconDex in your server, you must "
    f"read and accept the [Terms of Service]({settings.terms_of_service}).\n\n"
    "As a summary, these are the rules of the bot:\n"
    f"- No farming (spamming or creating servers for icons)\n"
    f"- Selling or exchanging icons "
    ...
    async def disable(self, interaction: discord.Interaction["BallsDexBot"]):
        ...
        if config.enabled:
            ...
            await interaction.response.send_message(
                ...
                f"available, but the spawn of new icons "
